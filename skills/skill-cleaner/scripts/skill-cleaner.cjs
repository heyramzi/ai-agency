...
var CLAUDE_HOME = ".claude/skills";
var OTHER_RUNTIME_HOMES = [
  ".agents/skills",
  ".opencode/skills",
  ".codex/skills",
  ".gemini/skills"
];
var SKILL_HOMES = [CLAUDE_HOME, ...OTHER_RUNTIME_HOMES];
var PLUGIN_HOMES = [".claude/plugins"];
var SKIP_DIRS = /* @__PURE__ */ new Set(["node_modules", ".git", "dist", "build", ".svelte-kit", ".next"]);
var MAX_DEPTH = 8;
function defaultRoots(cwd = process.cwd(), home = (0, import_node_os.homedir)(), opts = {}) {
  const homes = opts.allRuntimes ? SKILL_HOMES : [CLAUDE_HOME];
  return [cwd, ...[...homes, ...PLUGIN_HOMES].map((rel) => (0, import_node_path.join)(home, rel))].filter(
    (r) => (0, import_node_fs.existsSync)(r)
  );
}
...
function repoOf(fromPath) {
  let dir = (0, import_node_path.dirname)(fromPath);
  for (; ; ) {
    if ((0, import_node_fs.existsSync)((0, import_node_path.join)(dir, ".git"))) return dir;
    const up = (0, import_node_path.dirname)(dir);
    if (up === dir) return null;
    dir = up;
  }
}
function walkSkills(root, home = (0, import_node_os.homedir)()) {
  const found = [];
...
function danglingLinks(root) {
  ...
  const visit = (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      const link = safeLstat(child);
      if (!link) continue;
      if (link.isSymbolicLink() && !(0, import_node_fs.existsSync)(child)) {
  ...
  visit((0, import_node_path.resolve)(root), 0);
  return broken;
}
var insideSkillsDir = (path) => (0, import_node_path.dirname)(path).split(import_node_path.sep).includes("skills");
function safeReal(p) {
  try {
...
function safeLstat(p) {
  try {
    return (0, import_node_fs.lstatSync)(p);
  } catch {
    return null;
  }
}
...
function recoverFlatFields(raw) {
  const fields = {};
...
var stripQuotes = (value) => /^(["']).*\1$/.test(value) ? value.slice(1, -1) : value;
...
var RUNTIME_FIELDS = /* @__PURE__ */ new Set([
  "argument-hint",
...
function checkSkill(skill) {
  ...
  const add = (code, severity, message, fixable = false) => findings.push({ code, severity, message, paths: at, fixable });
  if (skill.raw === null) {
  ...
  if (metadata !== void 0) {
    const flat = metadata !== null && typeof metadata === "object" && !Array.isArray(metadata) && Object.values(metadata).every((v) => typeof v === "string");
    if (!flat) {
...
// src/cli.ts
var USAGE = `skill-cleaner - audit, consolidate and clean up Agent Skills
...
function main(argv) {
  let parsed;
  try {
    parsed = (0, import_node_util.parseArgs)({ ...CONFIG, args: argv });
  } catch (error) {
