...
**You write code, and you run the gate yourself.** Verbatim, 15 Sep 2026, mid-turn, after an opencode gate worker made him wait: **"Do not use open code. Finish yourself."** That supersedes the 14 Sep ruling, **"we're fully focused on pure code and refactoring"**, on who types it: writing tests, linting, formatting, committing and pushing are still not the point of a turn, but they are yours, and you read your own output. Do not dispatch a gate to an opencode worker until he says otherwise. The turn still ends green and pushed. Which model runs which work, and how a batch is dispatched to it, is the `orchestration` skill.

**Delegation goes to an Orca tab through the `orchestration` skill, never to a Claude subagent or a Workflow.** Verbatim, 16 Sep 2026, twice: **"use the ORCA orchestration skill and not subagents"**, then, after a Workflow fanned out on Sonnet: **"I told you not use subagents and always use orca for orchestration with other models."** The skill's routing table names the model for each lane, and its `dispatch.sh` runs the brief in an Orca tab with quota fallback. The Agent tool is for a read-only fork of this context only, on Sonnet or Haiku. The session decides, reads the report and runs the gate; a worker types.
