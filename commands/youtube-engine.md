...
video/
  ...
  channel.md       their runtime-to-views table, from session 1
  script.md        the current script, in beats
  levers.md        their lever list, from their own two bands
...
## Session 1, the numbers and the script (lessons 2, 3, 4)
...
**Say the age adjustment out loud.** Older videos have had longer to accumulate, so a raw
...
- Adding beats by lengthening the bullets that exist.
- Any outbound ask before 95% of runtime.
- Rewriting the hook because views are low. Nobody who would have watched got that far.
...
## Session 2, record and cut (lessons 5, 6)

### The order is the whole lesson

`video-coach` runs **before** `descript-script-edit`, always. Cut first and the evidence
about how they record is gone: the restarts, the abandoned openings, the block they talked
themselves out of. Export the transcript after the cut and every recording habit number is
...
1. `dscript grab <name>` decodes and archives the clipboard, then prints the state. They do
   Cmd+A Cmd+C first.
2. `dscript words` prints the indexed transcript, which is what the cut list is written
   against.
3. `dscript apply cuts.json` rebuilds the payload and loads it back onto the clipboard.
4. `dscript check` reads the clipboard back and proves what was pasted matches what was
   built. They do Cmd+A Cmd+V after it.
...
Expect the composition to shorten by more than the characters predict, because abandoned
takes each sit in their own silence.

### Refuse

- Counting restarts as a quality signal. They are a cost in seconds and the cut removes
  them completely.
- Charging the delivery for a fault in the plan. A script under the beat floor is why the
  take had to improvise.
- Trusting a grep. It finds glued words and finds nothing about a surviving restart, which
  needs the transcript read as prose. Read it.
...
## Session 3, packaging and what comes out of it (lessons 7, 8, 9)

### The thumbnail, and the bands that make a trait a lever
...
**The file the editor exports is the file that uploads.** A re-encode whose best case is no
change and whose worst case is loss is not a quality step. Keep filename discipline by
copying or renaming. If an upload path has a size ceiling, change the path, not the file.

For every motion graphic, ask one question: what does the viewer know after this clip that
they did not know from the sentence. If the answer is the same thing but with shapes, put
...
overlay that inherits an h264 config renders clean, exits zero and arrives with a black
rectangle where the transparency should be. Nothing catches it except opening the file over
footage, so open it.

The frame is shared: the bottom 90 pixels belong to YouTube's progress bar and controls, and
the top right is where the cards teaser appears.

### The Shorts and the posts
...
scene, whether it carries an image, whether it ends on the idea. A count is necessary and it
is not sufficient.
...
1. **Their numbers, never the course's.** Every measurement gets re-run on their channel,
   and you never invent one they have not taken.
